			...
			struct stats_record *stats_prev)
{
	...
	for (int action_num = 0; action_num < 5; ++action_num)
	{
		char *fmt = "%-12s %'11lld pkts (%'10.0f pps)"
			" %'11lld bytes (%'6.0f bits/s)"
			" period:%f\n";
		const char *action = action2str(action_num);
		rec  = &stats_rec->stats[action_num];
		prev = &stats_prev->stats[action_num];
		...
		bitsps  = 8 * bytes / period;

		printf(fmt, action, rec->total.rx_packets, pps, rec->total.rx_bytes, bitsps, period);
	}
}
